...
export interface OperationsRepository {
  ...
  createWorkflow(input: Omit<MaxxWorkflow, "id" | "active" | "createdAt" | "updatedAt">): Promise<MaxxWorkflow>;
...
function materializeWorkflow(input: Omit<MaxxWorkflow, "id" | "active" | "createdAt" | "updatedAt">): MaxxWorkflow {
  const timestamp = now();
  return { id: randomUUID(), ...input, active: true, createdAt: timestamp, updatedAt: timestamp };
}
...
export class MemoryOperationsRepository implements OperationsRepository {
  ...
  async createWorkflow(input: Omit<MaxxWorkflow, "id" | "active" | "createdAt" | "updatedAt">) { const x = materializeWorkflow(input); this.workflows.set(x.id, x); return { ...x }; }
...
class SupabaseOperationsRepository implements OperationsRepository {
  ...
  async listConnections(operatorId: string) {
    const rows = await this.request<Array<Record<string, unknown>>>(`maxx_connections?operator_id=eq.${encodeURIComponent(operatorId)}&order=updated_at.desc`);
    return rows.map(mapConnection);
  }
  async getConnection(operatorId: string, id: string) {
    const rows = await this.request<Array<Record<string, unknown>>>(`maxx_connections?id=eq.${encodeURIComponent(id)}&operator_id=eq.${encodeURIComponent(operatorId)}&limit=1`);
    return rows[0] ? mapConnection(rows[0]) : undefined;
  }
  async createConnection(input: Omit<MaxxConnection, "id" | "status" | "createdAt" | "updatedAt">) {
    const x = materializeConnection(input);
    const rows = await this.request<Array<Record<string, unknown>>>("maxx_connections", { method: "POST", body: JSON.stringify(toConnectionRow(x)) });
    return mapConnection(rows[0]);
  }
  async listWorkflows(operatorId: string) {
    const rows = await this.request<Array<Record<string, unknown>>>(`maxx_workflows?operator_id=eq.${encodeURIComponent(operatorId)}&order=updated_at.desc`);
    return rows.map(mapWorkflow);
  }
  async getWorkflow(operatorId: string, id: string) {
    const rows = await this.request<Array<Record<string, unknown>>>(`maxx_workflows?id=eq.${encodeURIComponent(id)}&operator_id=eq.${encodeURIComponent(operatorId)}&limit=1`);
    return rows[0] ? mapWorkflow(rows[0]) : undefined;
  }
  async createWorkflow(input: Omit<MaxxWorkflow, "id" | "active" | "createdAt" | "updatedAt">) {
    const x = materializeWorkflow(input);
    const rows = await this.request<Array<Record<string, unknown>>>("maxx_workflows", { method: "POST", body: JSON.stringify(toWorkflowRow(x)) });
    return mapWorkflow(rows[0]);
  }
  async matchingEventWorkflows(operatorId: string, eventType: string) {
    const rows = await this.request<Array<Record<string, unknown>>>(`maxx_workflows?operator_id=eq.${encodeURIComponent(operatorId)}&active=eq.true&trigger_type=eq.event&trigger_value=eq.${encodeURIComponent(eventType)}&order=updated_at.desc`);
    return rows.map(mapWorkflow);
  }
  async listRefinements(operatorId: string) {
    const rows = await this.request<Array<Record<string, unknown>>>(`maxx_refinement_proposals?operator_id=eq.${encodeURIComponent(operatorId)}&order=updated_at.desc`);
    return rows.map(mapRefinement);
  }
  async createRefinement(input: Omit<MaxxRefinementProposal, "id" | "status" | "createdAt" | "updatedAt">) {
    const x = materializeRefinement(input);
    const rows = await this.request<Array<Record<string, unknown>>>("maxx_refinement_proposals", { method: "POST", body: JSON.stringify(toRefinementRow(x)) });
    return mapRefinement(rows[0]);
  }
  async setRefinementStatus(operatorId: string, id: string, status: MaxxRefinementProposal["status"]) {
    const rows = await this.request<Array<Record<string, unknown>>>(`maxx_refinement_proposals?id=eq.${encodeURIComponent(id)}&operator_id=eq.${encodeURIComponent(operatorId)}`, { method: "PATCH", body: JSON.stringify({ status, updated_at: now() }) });
    return rows[0] ? mapRefinement(rows[0]) : undefined;
  }
}
...
function mapWorkflow(row: Record<string, unknown>): MaxxWorkflow { return { id: String(row.id), operatorId: String(row.operator_id), name: String(row.name), pupId: String(row.pup_id), objective: String(row.objective), expectedProof: String(row.expected_proof), triggerType: row.trigger_type as MaxxWorkflow["triggerType"], triggerValue: row.trigger_value == null ? null : String(row.trigger_value), active: Boolean(row.active), createdAt: String(row.created_at), updatedAt: String(row.updated_at) }; }
function toWorkflowRow(x: MaxxWorkflow) { return { id: x.id, operator_id: x.operatorId, name: x.name, pup_id: x.pupId, objective: x.objective, expected_proof: x.expectedProof, trigger_type: x.triggerType, trigger_value: x.triggerValue, active: x.active, created_at: x.createdAt, updated_at: x.updatedAt }; }
...
export async function registerOperationsHubRoutes(app: FastifyInstance, config: MaxxConfig, repository: OperationsRepository = createOperationsRepository(config)) {
  ...
  app.post("/v1/pups/:sourcePupId/delegate", async (request, reply) => {
    ...
    const response = await app.inject({ method: "POST", url: "/v1/pup-handoffs", headers: forwardedHeaders(request), payload: { sourcePupId, targetPupId: parsed.data.targetPupId, instruction: `${parsed.data.objective}\nExpected proof: ${parsed.data.expectedProof}\nReturn the proof or stop at the existing MAXX approval boundary.` } });
  ...
  app.post("/v1/pups/:sourcePupId/fresh-specialist", async (request, reply) => {
    ...
    const instruction = [
      ...
      `Temporary role: ${parsed.data.role}`,
      `Objective: ${parsed.data.objective}`,
      parsed.data.context ? `Task packet:\n${parsed.data.context}` : "Task packet: no extra context supplied.",
      `Expected proof: ${parsed.data.expectedProof}`,
      "Treat this packet as the complete task context. Do not rely on prior chat context, do not delegate again, and do not expand permissions.",
      "Complete the bounded task or stop for the existing MAXX approval boundary. Return evidence to the source Pup.",
    ].join("\n");
  ...
  app.post("/v1/workflows", async (request, reply) => {
    ...
    if (parsed.data.triggerType === "interval") {
      ...
      const patch = await app.inject({
        method: "PATCH",
        url: `/v1/pups/${encodeURIComponent(parsed.data.pupId)}`,
        headers: forwardedHeaders(request),
        payload: { routineEveryMinutes: minutes, routinePrompt: `${parsed.data.objective}\nExpected proof: ${parsed.data.expectedProof}` },
      });
      if (patch.statusCode !== 200) return reply.code(409).send({ error: "MAXX could not attach this routine to the selected Pup", detail: patch.json() });
    }
