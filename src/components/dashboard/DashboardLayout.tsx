...
const recoveryNav = [
  { path: "/control/command", label: "Command", icon: Command },
  { path: "/control/live", label: "Live", icon: Activity },
  { path: "/control/missions", label: "Missions", icon: KanbanSquare },
  { path: "/control/crm", label: "CRM", icon: BriefcaseBusiness },
];

const intelligenceNav = [
  { path: "/control/approvals", label: "Approvals", icon: ShieldCheck },
  { path: "/control/skills", label: "Skills", icon: Sparkles },
  { path: "/control/usage", label: "Usage", icon: Gauge },
  { path: "/control/settings", label: "Settings", icon: Settings },
];
...
function TowerNav() {
  ...
  const isActive = (path: string) => {
    return location.pathname === "/control" && path.endsWith("/command")
      ? true
  ...
  return (
    <div className="flex items-center gap-xs xs:gap-2 sm:gap-3">
      ...
      <div className="min-w-0">
        <p className="text-[8px] xs:text-[9px] sm:text-[10px] font-semibold uppercase tracking-[0.2em] xs:tracking-[0.24em] sm:tracking-[0.28em] text-sidebar-foreground/50 truncate">Advanced</p>
    ...
    <SidebarProvider>
      <div className="min-h-screen bg-background text-foreground w-full flex">
        <Sidebar collapsible="offcanvas" variant="sidebar">
          ...
          <SidebarFooter>
            <Link to="/dashboard" className="rounded-xl border border-sidebar-border bg-sidebar-accent/20 px-3 py-2 text-xs font-medium text-sidebar-foreground/75 transition hover:bg-sidebar-accent">
              Back to Stacy view
            </Link>
            <div className="rounded-lg xs:rounded-xl sm:rounded-2xl border border-sidebar-border bg-sidebar-accent/20 p-2 xs:p-2.5 sm:p-3.5">
              <div className="flex items-center gap-1.5 xs:gap-2 text-[10px] xs:text-[11px] sm:text-xs font-medium text-sidebar-foreground">
                <span className={`h-1.5 xs:h-2 w-1.5 xs:w-2 rounded-full ${status === "online" ? "bg-emerald-500" : status === "degraded" ? "bg-amber-500" : "bg-red-500"}`} />
            ...
            <button onClick={() => signOut()} className="flex items-center gap-1.5 xs:gap-2 rounded-lg xs:rounded-lg sm:rounded-xl px-2 xs:px-2.5 sm:px-3 py-1.5 xs:py-2 sm:py-2 text-[10px] xs:text-[11px] sm:text-xs font-medium text-sidebar-foreground/60 transition hover:bg-sidebar-accent hover:text-sidebar-foreground">
          ...
          <header className="sticky top-0 z-20 flex h-14 xs:h-16 items-center justify-between gap-2 xs:gap-3 border-b border-border bg-background/85 px-xs xs:px-sm sm:px-md lg:px-lg backdrop-blur-2xl">
            <div className="flex items-center gap-xs xs:gap-2 sm:gap-3 min-w-0 flex-1">
              ...
              <div className="min-w-0 flex-1">
                <p className="text-[8px] xs:text-[9px] sm:text-[10px] font-semibold uppercase tracking-[0.2em] xs:tracking-[0.22em] sm:tracking-[0.24em] text-foreground/50 truncate">Advanced operator view</p>
