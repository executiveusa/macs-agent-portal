...
export const ShellLayout: React.FC<ShellLayoutProps> = ({ children, introComplete = false }) => {
  const audioRef = useRef<HTMLAudioElement>(null);

  useEffect(() => {
    if (introComplete && audioRef.current) {
      audioRef.current.volume = 0.35;
      audioRef.current.play().catch(() => {
        // Autoplay blocked by browser policy — user interaction will be needed.
      });
    }
  }, [introComplete]);
  ...
  return (
    <div className="relative min-h-screen bg-maxx-bg text-white overflow-x-hidden selection:bg-maxx-cyan selection:text-black">
      ...
      {/* Soundtrack — drop the MP3 at public/audio/maxx-intro.mp3 to activate */}
      <audio ref={audioRef} src="/audio/maxx-intro.mp3" loop preload="none" className="hidden" />
      ...
      <nav className="fixed top-0 left-0 w-full z-40 p-6 flex justify-between items-center mix-blend-difference text-white">
        ...
        <div className="space-x-4 hidden md:block">
          ...
          <a href="#car-intro" className="hover:text-maxx-cyan transition-colors">ASSET</a>
          <a href="#car" className="hover:text-maxx-cyan transition-colors">ORDNANCE</a>
